...
namespace businessLogic.SearchEngines
{
    internal class GoogleSearchEngine : BaseSearchEngine, ISearchEngine
    {
        public SearchEngineResultsList Search(string query)
        {
            ...
            var apiKey = "AIzaSyB8kNz-iLRMinVRviNJHtJUkgPPOAx7mIk";
            var cx = "009511415247016879030:smaostb1cxe";
            var count = 1;
            uint start = 1;
                ...
                for (var i = 0; i < 1; i++)
                {
                    var result =
                        webClient.DownloadString(
                            string.Format(
                                "https://www.googleapis.com/customsearch/v1?key={0}&cx={1}&q={2}&start={3}&alt=json&cr=us",
                                apiKey, cx, query, start));
